...
const SlideViewer: React.FC<SlideViewerProps> = ({ slides }) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto my-6 relative bg-gray-50 p-4 rounded-xl shadow-inner">
            <Carousel 
                arrows 
                prevArrow={<div className="text-gray-400 hover:text-blue-500 text-3xl"><LeftOutlined /></div>} 
                nextArrow={<div className="text-gray-400 hover:text-blue-500 text-3xl"><RightOutlined /></div>}
                    ...
                    <div key={index} className="px-10 py-4 outline-none">
                        <Card className="h-[400px] flex flex-col justify-center border-t-4 border-t-blue-500 shadow-lg rounded-2xl bg-white">
                            <div className="text-center mb-6">
                                <Title level={2} className="text-blue-700 m-0">{slide.title}</Title>
                            </div>
