...
const QuizViewer: React.FC<QuizViewerProps> = ({ quizzes }) => {
        ...
        return (
            <Card className="max-w-3xl mx-auto my-6 shadow-md border-t-4 border-t-green-500">
                <Result
                    ...
                    title={percentage >= 50 ? "Tuyệt vời! Bạn đã hoàn thành bài tập." : "Bạn cần cố gắng hơn nhé!"}
                    subTitle={`Bạn đã trả lời đúng ${score}/${quizzes.length} câu hỏi.`}
                    extra={[
                        <Progress key="prog" type="circle" percent={percentage} size={120} className="mb-6 block mx-auto" />,
                        <Button key="restart" type="primary" onClick={handleRestart} size="large">
                            Làm lại bài
                        </Button>
                    ]}
                />
    ...
    return (
        <Card className="max-w-3xl mx-auto my-6 shadow-md border-t-4 border-t-blue-500">
            <div className="flex justify-between items-center mb-6 text-gray-500">
                <Text strong className="text-blue-600">Câu hỏi {currentIdx + 1} / {quizzes.length}</Text>
            ...
            >
                {currentQuiz.options.map((option, idx) => {
                    let optionClass = "p-4 border rounded-lg transition-all w-full text-base";
                    if (isSubmitted) {
                        if (option === currentQuiz.answer) {
                            optionClass += " border-green-500 bg-green-50 animate-pulse-once";
                        } else if (option === selectedOption && option !== currentQuiz.answer) {
                    ...
                    return (
                        <Radio key={idx} value={option} className={optionClass}>
                            <div className="flex w-full items-center whitespace-normal break-words">
                                ...
                                {isSubmitted && option === selectedOption && option !== currentQuiz.answer && (
            ...
            {isSubmitted && (
                <div className={`p-4 rounded-lg mb-6 ${isCorrect ? 'bg-green-100 border border-green-300' : 'bg-orange-100 border border-orange-300'}`}>
                    <Text strong className={isCorrect ? 'text-green-700' : 'text-orange-700'}>
                        {isCorrect ? 'Chính xác! 🎉' : 'Opps! Chưa đúng rồi.'}
                    </Text>
                    <Paragraph className="mt-2 text-gray-700 mb-0">
                        <Text strong>Giải thích: </Text>{currentQuiz.explanation}
                    </Paragraph>
                </div>
            )}
