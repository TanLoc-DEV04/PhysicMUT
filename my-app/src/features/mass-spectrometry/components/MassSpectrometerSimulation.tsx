...
export default function MassSpectrometerSimulation({
  gameStateRef,
  msCallbacks,
  imperativeRef,
}: MSSimulationProps) {
  ...
  useEffect(() => {
    ...
    const bendRadius = 12;
    const analyzer = new MassAnalyzer(
    ...
    const particleSystem = new ParticleSystem(
      ...
      (mass, charge, actualR, isotopeName) => {
        ...
        if (hitBin1) {
          detector.flashBin(0, true);
          detector.addCount(0);
        } else if (hitBin2) {
          detector.flashBin(1, true);
          detector.addCount(1);
        } else {
          detector.flashBin(0, false);
        }
      },
    );
    ...
    const syncParamsFromGameState = () => {
      ...
      if (gs.mode === "mission1") {
        ...
        controller.params.bin1TargetR = computeVisualR(
          12.0,
          1,
          voltage,
          magneticField,
        );
        controller.params.bin2TargetR = computeVisualR(
          14.003,
          1,
          voltage,
          magneticField,
