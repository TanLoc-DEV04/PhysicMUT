...
const MISSION_CARDS = [
  {
    ...
    color: "#3b82f6",
    story:
      "Hệ thống bị lệch chuẩn! Ion ¹²C⁺ đang bắn vào vách ống thay vì vào Máy dò 1. Kéo thanh trượt Từ trường B để uốn quỹ đạo cho đúng.",
    badge: "CALIBRATOR",
  },
  {
    ...
    color: "#10b981",
    story:
      "Mẫu khảo cổ lẫn lộn C-12 và C-14 phóng xạ. Chỉnh B và U để tách hai loại hạt vào đúng rỗ hứng. Thu 30 hạt mỗi loại!",
  ...
  },
  {
    ...
    color: "#8b5cf6",
    story:
      "Bệnh viện cần I-131 để xạ trị, nhưng bị lẫn với I-127. Chênh lệch rất nhỏ (ΔR ≈ 2%). Cần tinh chỉnh cực kỳ chính xác!",
    badge: "NUCLEAR_MED_TECH",
  },
];

// ─── Mass Spectrum Chart ──────────────────────────────────────────────────────
function SpectrumChart({ data }: { data: Record<string, number> }) {
      ...
      <div className="msg-spectrum-bars">
        {entries.map(([key, count]) => (
          <div key={key} className="msg-spectrum-bar-col">
            ...
            />
            <span className="msg-spectrum-label">
              {key.replace("Bin1_", "").replace("Bin2_", "")}
            </span>
          </div>
        ))}
        {entries.length === 0 && (
          <span className="msg-no-data">Chưa có dữ liệu...</span>
        )}
      </div>
    </div>
  );
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function MSGame() {
  const [gameState, setGameState] = useState<MSGameState>(() =>
    createInitialMSGameState(),
  );
  ...
  const msCallbacks = useRef<MSCallbacks>({
    ...
    onMissionComplete: (mode) => {
      if (mode === "mission1") {
        ...
        pushToast(
          "🎉 Đã tách đủ 30 hạt mỗi loại! Nhiệm vụ hoàn thành!",
          "badge",
        );
    ...
    <div className="msg-root">
        ...
        <div className="msg-modal-bg">
          <div className="msg-modal">
            <div className="msg-modal-header">
              <h2>
                <RocketOutlined style={{ color: "#60a5fa", marginRight: 8 }} />
                MÁY QUANG PHỔ KHỐI — CHỌN NHIỆM VỤ
              </h2>
              <p className="msg-streak">
            ...
            <div className="msg-mission-cards">
              {MISSION_CARDS.map((m) => (
                ...
                >
                  ...
                  {hasBadge(m.badge) && (
                    <span className="msg-badge-tick">✅</span>
                  )}
                </button>
              ))}
            </div>
      ...
      {!showModal && (
        <div className="msg-hud-top">
          <button
            className="msg-btn msg-btn-icon"
      ...
      {gameState.mode === "mission1" && !showModal && (
        <div className="msg-panel msg-m1-panel">
          {m1Done ? (
            ...
            <>
              <p className="msg-target">
                <AimOutlined /> Đưa ¹²C⁺ vào Máy dò 1
              </p>
              <div className="msg-m1-steps">
                <div className="msg-step">
                  <span className="msg-step-num">1</span>
                  <span>
                    Chọn <strong>C-12</strong> trong mục{" "}
                    <em>Sample Properties</em>
                  </span>
                </div>
                <div className="msg-step">
                  <span className="msg-step-num">2</span>
                  <span>
                    Kéo <strong>Magnetic Field B</strong> đến ≈{" "}
                    <strong>{m1HintB} T</strong>
                  </span>
                </div>
                <div className="msg-step">
                  <span className="msg-step-num">3</span>
                  <span>
                    Quan sát quỹ đạo — hạt phải lọt vào rổ xanh lá đầu tiên
                  </span>
                </div>
              </div>
              <details className="msg-hint-detail">
                <summary>
                  <BulbOutlined /> Công thức
                </summary>
      ...
      {gameState.mode === "mission2" && !showModal && (
        <div className="msg-panel msg-m2-panel">
          <p className="msg-target">
            <ExperimentOutlined /> Tách C-12 (Rổ 1) và C-14 (Rổ 2)
          </p>
          ...
          <details className="msg-hint-detail">
            ...
            <p>
              R ∝ √m (cùng q, U, B) → C-14 có R lớn hơn C-12 khoảng{" "}
              {(Math.sqrt(14 / 12) * 100 - 100).toFixed(1)}%.
              <br />
              Tăng B để phân tách rộng hơn.
            </p>
          </details>
        </div>
      )}
      ...
      {gameState.mode === "mission3" && !showModal && (
        <div className="msg-panel msg-m3-panel">
          <p className="msg-target">
            <MedicineBoxOutlined /> Tách I-127 (Rổ 1) và I-131 (Rổ 2)
          </p>
          <div className="msg-progress-row">
            ...
            <div className="msg-progress-item" style={{ color: "#ff8822" }}>
              ...
              <span>
                {m3Counts.bin2}/{M3_TARGET_COUNT}
              </span>
            </div>
          </div>
          <details className="msg-hint-detail">
            <summary>
              <BulbOutlined /> Công thức & Chiến lược
            </summary>
            <p>
              m/q = B²R² / (2U)
              <br />
              ΔR/R = Δm/(2m) ≈ {((4 / (2 * 131)) * 100).toFixed(2)}%<br />→ Cần
              B lớn và U nhỏ để phân tách rõ hơn.
            </p>
          </details>
        </div>
      )}
      ...
      {showBadges && (
        <div className="msg-drawer">
          ...
          {Object.values(MS_BADGES).map((b) => (
            <div
              key={b.id}
              className={`msg-badge-row ${hasBadge(b.id) ? "msg-badge-earned" : "msg-badge-locked"}`}
            >
      ...
      {showSpectrum && (
        <div className="msg-drawer msg-drawer-left">
          <div className="msg-drawer-header">
            <h3>
              <BarChartOutlined style={{ marginRight: 6 }} /> Phổ Khối
            </h3>
      ...
      <div className="msg-toast-container">
        {toasts.map((t) => (
          <div key={t.id} className={`msg-toast msg-toast-${t.type}`}>
            {t.text}
          </div>
        ))}
      </div>
