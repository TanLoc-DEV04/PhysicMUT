...
import { MEDIUMS, HEARING_RANGES, NOTE_PRESETS, type NotePreset, type LSGameState } from './types';
...
export interface LSSimulationProps {
    ...
    imperativeRef?: React.MutableRefObject<{ setMedium: (m: string) => void } | null>;
}

const LoudspeakerSimulation: React.FC<LSSimulationProps> = ({
    gameStateRef,
    callbacks,
    imperativeRef,
}) => {
    ...
    useEffect(() => {
        ...
        const particleMat = new THREE.PointsMaterial({
            size: 0.2, color: 0x88ccff, transparent: true, opacity: 0.6, sizeAttenuation: true
        });
        ...
        const updateBioRadar = () => {
            ...
            const matchPreset = NOTE_PRESETS.find(n => Math.abs(n.frequency - f) < 0.1);

            let statusText  = 'Sóng lan truyền bình thường';
            let statusColor = '#00ffcc';
            if (!medium.canPropagate) { statusText = 'CHÂN KHÔNG: Không có âm thanh!'; statusColor = 'red'; }
            else if (matchPreset)     { statusText = `${matchPreset.note} - ${matchPreset.description}`; statusColor = '#fff'; }

            setRadarData({
                freq: f, medium: medium.name, statusText, statusColor,
                animals: HEARING_RANGES.map(a => ({ ...a, active: f >= a.min && f <= a.max }))
            });
        ...
        const updateMediumVisuals = (mKey: string) => {
            ...
            if (mKey === 'Air')    { scene.background = new THREE.Color(0xf5f5f5); scene.fog = new THREE.Fog(0xf5f5f5, 10, 60); particleMat.color.setHex(0x3b82f6); }
            else if (mKey === 'Water')  { scene.background = new THREE.Color(0x001133); scene.fog = new THREE.Fog(0x001133, 10, 60); particleMat.color.setHex(0x88ccff); }
            else /* Vacuum */ { scene.background = new THREE.Color(0x111111); scene.fog = new THREE.Fog(0x111111, 10, 60); }
            updateBioRadar();
        };
        ...
        const gui = new GUI({ container: mountRef.current, title: 'ĐIỀU KHIỂN LOA', width: 290 });
        ...
        const ctrlPower = gui.add(state, 'isPlaying').name('Nguồn điện (Power)').onChange((v: boolean) => { if (!v) stopAudio(); });
        addIcon(ctrlPower, <PlayCircleOutlined style={{marginRight:6, color:'#22c55e'}}/>, 'Nguồn điện (Power)');

        const folderPhys = gui.addFolder('Vật Lý & Tín Hiệu');
        addIcon(folderPhys, <ThunderboltOutlined style={{marginRight:6, color:'#f59e0b'}}/>, 'Vật Lý & Tín Hiệu');
        folderPhys.add(state, 'frequency', 1, 25000, 1).name('Tần số f (Hz)').listen().onChange(updateBioRadar);
        folderPhys.add(state, 'currentAmplitude', 0, 5, 0.01).name('Cường độ I₀ (A)').listen().onChange((v: number) => {
        ...
        addIcon(folderNotes, <SoundOutlined style={{marginRight:6, color:'#a78bfa'}}/>, 'Mẫu Âm Thanh');
        folderNotes.add(state, 'currentNote', ['Custom', ...NOTE_PRESETS.map(n => n.note)]).name('Chọn nốt').onChange((val: string) => {
            if (val === 'Custom') stopAudio();
            else { const p = NOTE_PRESETS.find(n => n.note === val); if (p) playNote(p); }
        });

        const folderEnv = gui.addFolder('Môi Trường');
        addIcon(folderEnv, <GlobalOutlined style={{marginRight:6, color:'#38bdf8'}}/>, 'Môi Trường');
        ...
        addIcon(infoF, <BarChartOutlined style={{marginRight:6, color:'#4ade80'}}/>, 'Thông số');
        ...
        const animate = () => {
            ...
            switch (state.waveform) {
                case 'Sine':     disp = Math.sin(phi); break;
                case 'Square':   disp = Math.sign(Math.sin(phi)); break;
                case 'Triangle': disp = Math.asin(Math.sin(phi)) / (Math.PI / 2); break;
                case 'Pulse':    disp = Math.sin(phi) > 0.9 ? 1 : 0; break;
            }
            ...
            if (gs.mode === 'mission3' && !m3WinFired && nowVacuum && !prevVacuum && state.isPlaying) {
                m3WinFired = true;
    ...
    return (
        <div className="w-full h-full relative">
            ...
            <div className="absolute bottom-5 left-5 w-[280px] bg-slate-900/90 text-[#00ffcc] p-4 rounded-lg border border-[#4fd1c5] font-mono shadow-lg pointer-events-auto">
                <h3 className="font-bold text-base mb-2 text-[#4fd1c5] border-b border-gray-600 pb-1">RADAR ÂM THANH</h3>
                ...
                <div className="flex justify-between mb-3 text-sm">
                    ...
                    <span className="font-bold" style={{ color: radarData.statusColor }}>{radarData.statusText}</span>
                </div>
                <hr className="border-gray-700 my-2" />
                <div className="space-y-1 max-h-36 overflow-y-auto">
                    {radarData.animals.map((a, i) => (
                        <div key={i} className={`text-xs transition-all duration-300 ${a.active ? 'opacity-100 text-yellow-300 font-bold' : 'opacity-30 text-gray-400'}`}>
